...
| Workflow | 20 custom agentic pipeline skills |
...
| `.opencode/skills/` | 20 agentic workflow skill definitions | Yes |
...
> **Permissions:** The project-level `opencode.json` controls plugins, MCP servers, and skill permissions (auto-allow all skills). Build-mode permissions (edit/write/bash) are set globally at `~/.config/opencode/opencode.json` (see [§2](#2-configure-opencode-global-permissions)). To override per-project, add an `agent.build.permission` block to the root `opencode.json`.
