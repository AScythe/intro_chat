// useDemoSimulation.test.ts [CLEANUP]
// Description: Tests for useDemoSimulation hook — demo flag toggles simulation behavior

import { describe, it, expect } from 'vitest';
import { renderHook } from '@testing-library/react';
import { useDemoSimulation } from '@/hooks/useDemoSimulation';

describe('useDemoSimulation', () => {
  it('provides sample users by room name when enabled', () => {
    const { result } = renderHook(() => useDemoSimulation(true));
  ...
  it('returns empty when demo is disabled', () => {
    const { result } = renderHook(() => useDemoSimulation(false));
