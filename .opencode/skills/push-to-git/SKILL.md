...
### 3. Present Groups
Show the user the proposed grouping as a numbered list. Use the `question` tool to ask: "Does this grouping look correct?" Provide clickable `options` with `label` ("Looks good", "Rearrange groups", "Customize") and `description` fields. Rely on the auto-added "Type your own answer" for custom input.
...
6. Use the `question` tool to ask for approval — provide clickable `options` with `label` ("Accept & commit", "Edit message", "Skip this group") and `description` fields
7. If "Edit message": the user types their revision in the auto-added "Type your own answer" field, then commit
